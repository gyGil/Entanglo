...
--  	end if;
	...
	_columnString := 'id SERIAL, tableuuid VARCHAR(100) NOT NULL, jsondata JSONB NOT NULL, 
				rawdataprofile JSONB NOT NULL, dataprofile JSONB, removed BOOLEAN DEFAULT FALSE, 
				tablecreated TIMESTAMP WITHOUT TIME ZONE NOT NULL, tableremoved TIMESTAMP WITHOUT TIME ZONE, ';
	...
	rawDataProfile := to_jsonb(_rawDataProfileString);
 	...
 	_insertString := 'INSERT INTO "' || _schema || '".' || _tableName || ' (tableuuid, jsondata, rawdataprofile, tablecreated)' || 
				' VALUES (''' || _tableUuid || ''', ''' || jsonData || ''', ''' || rawDataProfile || ''', ''' ||
					now()::timestamp without time zone || ''')';

	--raise exception 'insert string: %', _insertString;
